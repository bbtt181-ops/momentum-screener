"""
Generates a fresh, one-of-a-kind motivational header image for every email --
no two sends look the same, without depending on any external image service
being online (the earlier version picked from 6 fixed pre-made JPGs; this
replaces that with an effectively infinite pool generated on the fly).

Everything is drawn locally with Pillow + numpy (both already project
dependencies): a randomized two-color gradient background from a curated set
of tasteful hue families (so results stay professional-looking rather than
garish), plus one of several abstract "growth" motifs layered on top (bars,
arrow, mountain/summit, candlesticks, sunrise arc, trend dots).

No network calls, no filesystem dependency at send time -- generate_image_bytes()
returns ready-to-embed JPEG bytes.
"""
...
# images is effectively unbounded.
HUE_FAMILIES = [
...
def generate_image_bytes() -> bytes:
    """Builds one randomized gradient + motif JPEG and returns its raw bytes."""
    ...
    buf = io.BytesIO()
    combined.save(buf, format="JPEG", quality=87)
    return buf.getvalue()
